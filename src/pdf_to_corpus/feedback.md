Γεια σου! Αναλύοντας τα δύο παραδείγματα (ΦΕΚ 176/2025 και 175/2025) και το αρχείο `indexer.py`, εντόπισα αρκετά προβλήματα που εξηγούν γιατί το index που παράγεται είναι ελλιπές ή περιέχει "σκουπίδια".

### Τι έφταιγε στο αρχικό σου script:
1. **Λάθος Regex (Τα "Σκουπίδια"):** Το script σου έψαχνε για `ΜΕΡΟΣ` ή `ΚΕΦΑΛΑΙΟ` ακολουθούμενο από οποιοδήποτε γράμμα. Αυτό είχε ως αποτέλεσμα φράσεις του κειμένου όπως *"αποτελεί επουσιώδες **ΜΕΡΟΣ ΤΗΣ** γενικής δραστηριότητας"* να αναγνωρίζονται ως "ΜΕΡΟΣ ΤΗΣ" στο index! 
2. **Σύγχυση με τον Πίνακα Περιεχομένων (TOC):** Η προσπάθεια να εντοπίσεις και να αγνοήσεις τις σελίδες του TOC ήταν ασταθής. Αν το script έκανε λάθος και θεωρούσε μια κανονική σελίδα ως TOC, έχανες άρθρα. Αν θεωρούσε το TOC κανονική σελίδα, έπαιρνες τις σελίδες του TOC (π.χ. σελίδα 1) αντί για τις πραγματικές σελίδες των άρθρων.
3. **Καταστροφή Μορφοποίησης:** Η συνάρτηση `normalize_text` έκανε τα πάντα κεφαλαία και τα έγραφε έτσι στο Markdown, κάνοντάς το δυσανάγνωστο. Πρέπει να κάνουμε normalize για το "ψάξιμο", αλλά να κρατάμε το αρχικό κείμενο για το "τύπωμα".
4. **Λάθος Υπολογισμός Σελίδων (Ranges):** Ο κώδικας έβγαζε ranges τύπου `14-13` (επειδή έκανε `end_page - 1` τυφλά). Αν ένα άρθρο ξεκινάει και τελειώνει στην ίδια σελίδα, πρέπει απλά να γράφει τη σελίδα.
5. **Αδυναμία εξαγωγής Τίτλου:** Ο τίτλος συχνά κόβονταν απότομα ή περιελάμβανε άσχετο κείμενο. 

### Η Λύση

Έγραψα από την αρχή ένα πολύ πιο **έξυπνο και ανθεκτικό script**. 
Οι κύριες βελτιώσεις είναι:
* **Χρήση `Blocks` αντί για `Lines`:** Το PyMuPDF (`fitz`) έχει την επιλογή `get_text("blocks")` η οποία ομαδοποιεί λογικά τις παραγράφους. Αυτό βοηθάει απίστευτα στο να δούμε αν το "Άρθρο 1" και ο τίτλος του ανήκουν στην ίδια λογική ενότητα.
* **TOC Override (Αντικατάσταση TOC):** Αντί να μαντεύουμε ποιες σελίδες είναι TOC, σκανάρουμε όλο το έγγραφο. Επειδή το TOC είναι στην αρχή, αν βρούμε το "Άρθρο 1" στη σελίδα 1 (στο TOC) και μετά το ξαναβρούμε στη σελίδα 3 (στο σώμα του νόμου), το script απλά **αντικαθιστά** την παλιά εγγραφή. Έτσι κρατάει πάντα την πραγματική σελίδα του άρθρου!
* **Αυστηρά Regex:** Πλέον τα Μέρη και τα Κεφάλαια απαιτούν Λατινικούς/Ελληνικούς αριθμούς (π.χ. Α', Β', 1, 2) και όχι απλές λέξεις.
* **Ακριβής Τίτλος:** Εξάγει τον τίτλο με ακρίβεια, πιάνοντας το κείμενο ανάμεσα στο "ΝΟΜΟΣ ΥΠ' ΑΡΙΘΜ..." και το "Ο ΠΡΟΕΔΡΟΣ".

### Ο Νέος και Βελτιωμένος Κώδικας (`indexer.py`)

Αντικατάστησε όλο το περιεχόμενο του `indexer.py` με τον παρακάτω κώδικα:
...
### Τι θα δεις τώρα με τον νέο κώδικα στα Markdown αρχεία σου:
1. **Λογικό Range Σελίδων:** Πλέον θα δεις τα άρθρα να λένε `Σελίδες 5-13` (αν πιάνουν πολλές σελίδες) ή `Σελίδες 4` (αν το άρθρο τελειώνει στην ίδια σελίδα).
2. **Σωστή Αναγνώριση Κεφαλαίων/Μερών:** Απέκλεισα εντελώς τις προτάσεις κειμένου τύπου "μέρος της", χάρη στο `re_part = re.compile(r"^ΜΕΡΟΣ\s+([Α-ΩIVX]+|\d+)['’]?")` το οποίο απαιτεί Λατινικούς ή Ελληνικούς αριθμούς.
3. **Ούτε ένα χαμένο άρθρο:** Επειδή χρησιμοποιεί dictionary (`key = "ARTICLE_1"`), το TOC περνιέται πρώτο και όταν το script φτάσει στη σελίδα του **πραγματικού άρθρου**, το κάνει overwrite παίρνοντας τη σωστή σελίδα, εξαλείφοντας την ανάγκη να μαντεύουμε αν μια σελίδα είναι TOC ή όχι.
4. **Σωστός Τίτλος:** Τα Markdown αρχεία θα έχουν πεντακάθαρο τον τίτλο του νόμου μέσα στο κείμενο και στα metadata, χωρίς "Η ΠΡΟΕΔΡΟΣ ΤΗΣ ΔΗΜΟΚΡΑΤΙΑΣ" κλπ κολλημένα στο τέλος.

Τρέξ' το και το Index για τον AI Agent σου θα είναι έτοιμο, πεντακάθαρο και έτοιμο για RAG/Information Retrieval!